...
if [ "$BETA" = "TRUE" ]; then
  CHANGELOG_CONTENT=$("$SCRIPT_DIR/get_latest_beta_changelog.sh" "--repo" "$REPO" "--version" "$VERSION")
elif [ "$CANARY" = "TRUE" ]; then
  CHANGELOG_CONTENT=$("$SCRIPT_DIR/get_latest_canary_changelog.sh" "--repo" "$REPO" "--version" "$VERSION")
else
  CHANGELOG_CONTENT=$("$SCRIPT_DIR/get_latest_changelog.sh")
fi
...
if [ ${#CHANGELOG_CONTENT} -gt 4096 ]; then
  ...
  CHANGELOG_CONTENT+=$"\nFor the complete changelog, visit: https://github.com/cjlapao/common-go/releases/tag/v${VERSION}"
fi
